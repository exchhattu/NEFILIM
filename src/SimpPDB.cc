...
SimPDB::SimPDB(const std::string fragment_id, const vector<float> ca_xyzs) {
  size_t size_xyz   = ca_xyzs.size();
  mProteinFileName  = fragment_id.c_str();
  mNumResidue       = ca_xyzs.size() / 3;
  mCAlpha           = new float[size_xyz];
  assert(size_xyz > 0);
