...
void RMSDAndReplace(double** ref_xlist, //[3][nres],
                    double** mov_xlist, //[3][nres],
                    int n_list,
                    double* rmsd,
                    float* result) {
  ...
  for (int i = 0 ; i < n_list ; ++i) {
    a = (U[0][0] * mov_xlist[0][i] +
         U[0][1] * mov_xlist[1][i] +
         U[0][2] * mov_xlist[2][i]);
    b = (U[1][0] * mov_xlist[0][i] +
         U[1][1] * mov_xlist[1][i] +
         U[1][2] * mov_xlist[2][i]);
    c = (U[2][0] * mov_xlist[0][i] +
         U[2][1] * mov_xlist[1][i] +
         U[2][2] * mov_xlist[2][i]);
    result[3*i]   = (float)a;
 ...
 */
void setup_rotation(double** ref_xlist, //[3][nres],
                    double** mov_xlist, //[3][nres],
                    int n_list,
                    double R[3][3],
                    double* E0) {
  ...
  /* initialize */
  *E0 = R[0][0] = R[0][1] = R[0][2] = R[1][0] = R[1][1] = R[1][2] = R[2][0] =
    R[2][1] = R[2][2] = 0.0;
  ...
  for (int n = 0 ; n < n_list ; ++n) {
     ...
     */
    *E0 += (mov_xlist[0][n] * mov_xlist[0][n] +
            ref_xlist[0][n] * ref_xlist[0][n] +
            mov_xlist[1][n] * mov_xlist[1][n] +
            ref_xlist[1][n] * ref_xlist[1][n] +
            mov_xlist[2][n] * mov_xlist[2][n] +
            ref_xlist[2][n] * ref_xlist[2][n]);
    /*
     * correlation matrix R:
     *   R[i,j) = sum(over n): y(n,i) * x(n,j)
     *   where x(n) and y(n) are two vector sets
     */
    R[0][0] += mov_xlist[0][n] * ref_xlist[0][n];
    R[0][1] += mov_xlist[0][n] * ref_xlist[1][n];
    R[0][2] += mov_xlist[0][n] * ref_xlist[2][n];
    R[1][0] += mov_xlist[1][n] * ref_xlist[0][n];
    R[1][1] += mov_xlist[1][n] * ref_xlist[1][n];
    R[1][2] += mov_xlist[1][n] * ref_xlist[2][n];
    R[2][0] += mov_xlist[2][n] * ref_xlist[0][n];
    R[2][1] += mov_xlist[2][n] * ref_xlist[1][n];
    R[2][2] += mov_xlist[2][n] * ref_xlist[2][n];
  }
...
void calculate_rotation_rmsd(double** ref_xlist, //[3][nres]
                             double** mov_xlist, //[3][nres]
                             int n_list,
                             double U[3][3],
                             double* rmsd) {
...
void fast_rmsd(double** ref_xlist, //[3][nres],
               double** mov_xlist, //[3][nres],
               int n_list,
               double* rmsd) {
