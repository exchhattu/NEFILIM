...
DistMatrix::DistMatrix(map<string, vector<float> > fragments, float clustering_radius) {
  SimPDB* firstPDB = NULL;
  _DIST_RANGE_ZERO = DistRange(0.0);
  
  map<string, vector<float> >::iterator msvit=fragments.begin(); 
  firstPDB = new SimPDB(msvit->first, msvit->second);
  _mLen = static_cast<int>(msvit->second.size()/3);
  _mPOS1 = MatInit(3, _mLen);
  _mPOS2 = MatInit(3, _mLen);
  _nb_PDBs  = static_cast<int>(fragments.size()); 
  ...
  for (size_t i = 1 ; i < _nb_PDBs ; ++i) {
    ++msvit;
    (*_read_pdbs)[i] = new Stru(new SimPDB(msvit->first, msvit->second));
...
/**
 * SLA clustering
 * @param PDB_files:           all PDB filenames to read in
 * @param nb_clusters_to_find: number of partitions to extract from the set
 * @param clusters_found:      nb_clusters_to_find clusters, each cluster
 *                             is just a vector of indexes in PDB_files of the
 *                             PDBs that are in the same cluster
 */
DistMatrix::DistMatrix(const vector<string>& PDB_files,
  ...
  single._only_rank = true; // don't allocate the matrix

  for (size_t line_index = 0; line_index < PDB_files.size() ; ++line_index) {

    const string& pdb_name = PDB_files[line_index];
    if (not initialized) {
      firstPDB = new SimPDB(pdb_name.c_str(), false);
      _mLen = firstPDB->mNumResidue;
      _mPOS1 = MatInit(3, _mLen);
      _mPOS2 = MatInit(3, _mLen);
      initialized = true;
  ...
  _matrix = NULL;
  for (size_t i = 1 ; i < _nb_PDBs ; ++i) {
    (*_read_pdbs)[i] = new Stru(new SimPDB(PDB_files[i].c_str(),
                                           _mLen, single._use_URMSD));
  }
  _nb_ca_rmsd_calls = 0;
  _previous_i_used = numeric_limits<std::size_t>::max();
  _method = THEOBALD;
  single_linkage_agglomerative(nb_clusters_to_find, clusters_found);
}
...
// return (n / N) / (avg(RMSD_n) / avg(RMSD_N))
float DistMatrix::compute_D_ziru(vector<int>& cluster) {

  size_t end              = cluster.size();
  static float avg_RMSD_N = rand_sample_avg_rmsd(); // estimated
  float avg_RMSD_n        = 0.0;                    // to compute exactly
  ...
  if (end > 0) {
    ...
    for (size_t i = 1 ; i < end ; ++i) {
      avg_RMSD_n += get_or_compute(center, cluster[i]);
    }
    avg_RMSD_n /= float(end);
  }

  return (((float)end) / ((float)_nb_PDBs)) / (avg_RMSD_n / avg_RMSD_N);
}

// normalized packing score (average RMSD of one to all the others)
float DistMatrix::compute_packing(vector<int>& cluster) {

  size_t end        = cluster.size();
  float avg_packing = 0.0;

  if (end > 0) {
    for (size_t i = 0 ; i < end ; ++i) {
      for (size_t j = i + 1 ; j < end ; ++j) {
        avg_packing += get_or_compute(cluster[i], cluster[j]);
      }
    }
    avg_packing /= (float(end * (end - 1))) / 2.0;
  }

  return avg_packing;
}

void DistMatrix::estimate_packing(vector<int>& cluster,
                                  double* min_pack,
                                  double* max_pack) {

  *min_pack = *max_pack = 0.0;
  size_t end            = cluster.size();
  ...
  if (end > 0) {
    ...
    *min_pack /= double(end * (end - 1)) / 2.0;
    *max_pack /= double(end * (end - 1)) / 2.0;
  }
}
...
// fill 'neighbors' with elements from 'dists_to_ref' which are less
// than _clustering_radius from 'current'
void DistMatrix::find_neighbors(float dist_to_current,
                                size_t current,
                                vector< pair<float, size_t> >& dists_to_ref,
                                vector<size_t>& neighbors) {

  float left_bound  = dist_to_current - _clustering_radius;
  float right_bound = dist_to_current + _clustering_radius;
  size_t i = 0;

  while (i < dists_to_ref.size() and dists_to_ref[i].first < left_bound) {
    ++i;
  }
  while (i < dists_to_ref.size() and dists_to_ref[i].first <= right_bound) {
    if (current != dists_to_ref[i].second) {
      neighbors.push_back(dists_to_ref[i].second);
    }
    ++i;
  }
  sort(neighbors);
...
void DistMatrix::smart_init_with_sets(int nb_refs_to_use) {

  vector<size_t> refs;
  // access: [ref][elt]
  vector< vector< pair<float, size_t> > > dists_to_ref(nb_refs_to_use);
  // access: [elt][ref]
  vector< vector<float> > dist_to_ref;
  vector<bool> finished(_nb_PDBs, false);

  for (size_t i = 0 ; i < _nb_PDBs ; ++i) {
    dist_to_ref.push_back(vector<float>(nb_refs_to_use, 0.0));
  }
  // measure all to each ref
  for (int refnum = 0 ; refnum < nb_refs_to_use ; ++refnum) {

    size_t current_ref = get_next_reference();
    refs.push_back(current_ref);
    finished[current_ref] = true;

    for (size_t pdbnum = 0 ; pdbnum < _nb_PDBs ; ++pdbnum) {

      if (pdbnum != current_ref) {
        float carmsd = ca_rmsd(current_ref, pdbnum);
        set(current_ref, pdbnum, carmsd); // store it
        dist_to_ref[pdbnum][refnum] = carmsd;
        dists_to_ref[refnum].push_back(pair<float, size_t>
                                       (carmsd, pdbnum));
      }
    }
    // sort result vectors
    std::sort(dists_to_ref[refnum].begin(), dists_to_ref[refnum].end(),
              is_shorter);
  }
  // for all PDBs
  for (size_t pdbnum = 0 ; pdbnum < _nb_PDBs ; ++pdbnum) {
    ...
    vector<size_t> previous_candidates;

    if (not finished[pdbnum]) { // if current PDB was not a reference
      // find the set of PDBs which are within d of i by intersecting
      // sets found for each ref
      for (int refnum = 0 ; refnum < nb_refs_to_use ; ++refnum) {

        vector<size_t> maybe_candidates;

        find_neighbors(dist_to_ref[pdbnum][refnum], pdbnum,
                       dists_to_ref[refnum], maybe_candidates);
        if (refnum == 0) {
          previous_candidates = maybe_candidates;
          candidates          = maybe_candidates;
        } else {
          previous_candidates = candidates;
          candidates = vector<size_t>();
          intersect(maybe_candidates, previous_candidates, candidates);
        }
      }
      // prune out set by removing PDBs which are further than d
      // by using bounds first before real rmsd (and caching things)
      for (size_t j = 0 ; j < candidates.size() ; ++j) {

        DistRange& range = get(pdbnum, candidates[j]);
        ...
        if (not range._is_decided) {
          ...
          if (not single._no_lower_bound) {
            //cout << "using lower bound" << endl;
            lower_bound = lower_bound_carmsd(pdbnum, candidates[j]);
          }
          if (lower_bound > _clustering_radius) {
            range = DistRange(lower_bound, range._maxi, _clustering_radius);
          } else {
            ...
            if (not single._no_upper_bound) {
              //cout << "using upper bound" << endl;
              upper_bound = upper_bound_carmsd(pdbnum, candidates[j]);
            }
            if (upper_bound <= _clustering_radius) {
              range = DistRange(range._mini, upper_bound, _clustering_radius);
            } else {
              set(pdbnum, candidates[j], ca_rmsd(pdbnum, candidates[j]));
            }
          }
        }
      }
    ...
    if (single._auto_switch and can_switch and
      ...
      cout << "smart init decided:   "
           << 100*((float)(previous_nb_undecided - _nb_undecided) /
                   (float)_nb_pairs) << " \%\n";
      if (single._brute_fix) {
        previous_nb_undecided = _nb_undecided;
        brute_fix_ranges();
        cout << "brute_fix_ranges decided: "
             << 100*((float)(previous_nb_undecided - _nb_undecided) /
                     (float)_nb_pairs) << " \%\n";
      }
      previous_nb_undecided = _nb_undecided;
...
// can propagate exactly measured distances as well as ranges
void DistMatrix::propagate(size_t a) {
  ...
  for (size_t i = 0 ; i < _nb_PDBs ; ++i) {
    ...
    if (i != a and _nb_undecided_neighbors[i] > 0) {
      ...
      //     a is current reference
      size_t b       = i;
      DistRange& ab_range = get(a, b);
      float ab_min        = ab_range._mini;
      float ab_max        = ab_range._maxi;

      for (size_t j = i + 1 ; j < _nb_PDBs ; ++j) {

        if (j != a and _nb_undecided_neighbors[j] > 0) {

          // make range decidable or at least sharpen it
          size_t   c           = j;
          DistRange& ac_range  = get(a, c);
          float   ac_min       = ac_range._mini;
          float   ac_max       = ac_range._maxi;
          DistRange& old_range = get(b, c);
          // former new_mini for exact measures only
          //float new_mini       = max(old_range._mini, fabs(ac_min - ab_min));
          // new one that can propagate ranges also
          float new_mini       = max(old_range._mini,
                                     max(ac_min - ab_max,
                                         ab_min - ac_max));
          float new_maxi       = min(old_range._maxi, ac_max + ab_max);

          DistRange new_range  = DistRange(new_mini, new_maxi,
                                           _clustering_radius);
...
// FBR: maybe the best DM initialization is by propagating new references
//      as long as it updates more ranges than the insertion step,
//      decidability with a given threshold value is not adapted here

// single linkage agglomerative clustering:   consider closest distance
// complete linkage agglomerative clustering: consider furthest distance
// group average clustering:                  consider average distance

bool is_shorter_p_pf(pair< pair<size_t, size_t>, float> p1,
                     pair< pair<size_t, size_t>, float> p2) {
  return p1.second < p2.second;
}

void DistMatrix::single_linkage_agglomerative
(const int nb_clusters_to_find, map< size_t, vector<size_t> >& clusters_found) {

  if (nb_clusters_to_find < 1 || nb_clusters_to_find > (int)_nb_PDBs) {
    std::cerr << __FILE__ << ':' << __LINE__ << ':' << __FUNCTION__ << ':'
              << " bad param nb_clusters_to_find: " << nb_clusters_to_find
              << std::endl;
    exit(1);
  }

  // init
  vector<size_t> cluster; // PDB -> cluster
  for (size_t i = 0 ; i < _nb_PDBs ; ++i) {
    // at start, each PDB is in its own cluster
    cluster.push_back(i);
  }
  // compute RMSDs
  vector< pair< pair<size_t, size_t>, float> > rmsds;
  for (size_t i = 0 ; i < _nb_PDBs ; ++i) {
    for (size_t j = i + 1 ; j < _nb_PDBs ; ++j) {
      rmsds.push_back(pair< pair<size_t, size_t>, float>
                      (pair<size_t, size_t>(i, j), ca_rmsd(i, j)));
    }
  }
  // sort RMSDs
  std::sort(rmsds.begin(), rmsds.end(), is_shorter_p_pf);
  // extract clusters
  int nb_clusters = _nb_PDBs;
  for (size_t i = 0 ;
       i < rmsds.size() && nb_clusters > nb_clusters_to_find;
       ++i) {

    pair< pair<size_t, size_t>, float>& current = rmsds[i];
    size_t a = cluster[current.first.first];
    size_t b = cluster[current.first.second];
    size_t first_cluster  = min(a, b); // cluster of a
    size_t second_cluster = max(a, b); // cluster of b

    if (first_cluster != second_cluster) {
      //cout << first_cluster << " <- " << second_cluster << endl;
      // reassign second cluster members to the first one
      for (size_t j = 0 ; j < _nb_PDBs ; ++j) {
        if (cluster[j] == second_cluster) {
          cluster[j] = first_cluster;
        }
      }
      --nb_clusters; // just merged two of them
    }
  }
  // export discovered clusters
  for (size_t i = 0 ; i < cluster.size() ; ++i) {

    size_t ci = cluster[i];

    if (clusters_found.find(ci) == clusters_found.end()) {
      clusters_found[ci] = vector<size_t>();
    }
    clusters_found.find(ci)->second.push_back(i);
...
void DistMatrix::brute_fix_ranges() {

  for (size_t a = 0 ; a < _nb_PDBs ; ++a) {

    for (size_t b = a + 1 ; b < _nb_PDBs ; ++b) { // generate all ABs

      DistRange& ab = get(a, b);

      if (not ab._is_decided) {

        float current_mini = ab._mini;
        float current_maxi = ab._maxi;
        float current_dist;

        for (size_t x = 0 ; x < _nb_PDBs ; ++x) {

          if (x != a and x!= b) { // inspect all Xs != A and B

            // find a longer ab._mini
            current_dist = max(get(a, x)._mini - get(x, b)._maxi,
                               get(b, x)._mini - get(x, a)._maxi);
            if (current_dist > current_mini) {
              current_mini = current_dist;
              //cout << "longer mini" << endl;
            }
            // find a shorter ab._maxi
            current_dist = get(a, x)._maxi + get(b, x)._maxi;
            if (current_dist < current_maxi) {
              current_maxi = current_dist;
              //cout << "shorter maxi" << endl;
            }
          }
        }
        DistRange new_range = DistRange(current_mini, current_maxi,
                                        _clustering_radius);
        update_if_necessary(a, b, ab, new_range);
      }
    }
  }
}
...
// return the average pairwise carmsd dist for a random sample of the
// whole population
float DistMatrix::rand_sample_avg_rmsd() {

  float result = 0.0;
  vector<float> sampled_dists;

  cout << "sampling for rand_sample_avg_rmsd:" << endl;
  sample(sampled_dists);
  size_t end = sampled_dists.size();

  for (size_t k = 0 ; k < end ; ++k) {
    result += sampled_dists[k];
  }
  result /= float(end);
