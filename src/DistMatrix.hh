...
class DistMatrix {
 ...
 public:
  ...
  DistMatrix(map<string, vector<float> > fragments, float clustering_radius); 

  // DistMatrix(vector<float> , float clustering_radius);
  ...
  void brute_init();
  void smart_init();
  void smart_init_with_sets(int nb_refs_to_use);
  void csv_dump();
  void txt_dump();
